  ...
  async function fetchRelatedNews() {
    try {
      const response = await axios.get(
        `${API_URL}/article?pagenation=true&limit=4&type=img&newsType=breakingNews&status=online&topic=${encodeURIComponent(topic)}&excludeId=${encodeURIComponent(currentNewId)}`
      );
      ...
      {relatedNews.length > 0 && (
        <div className="details-main-related-new-area">
          ...
          <div className="details-main-related-new-area-cards">
            {relatedNews.slice(0, 3).map((relNews) => (
              <DetailsNewsCard key={relNews._id} data={relNews} />
            ))}
          </div>
        </div>
      )}
