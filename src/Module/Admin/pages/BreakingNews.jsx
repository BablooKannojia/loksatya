...
const Upload = () => {
  ...
  const [uploadLoading, setUploadLoading] = useState(false); // New state for upload button
  const [saveLoading, setSaveLoading] = useState(false); // New state for save button
  ...
  const onUpload = async () => {
    setUploadLoading(true); // Set upload loading state
    setLoading(true);
    ...
    try {
      const image = await axios.post(`${API_URL}/image`, formdata);

      await axios.post(`${API_URL}/article/${localStorage.getItem("id")}`, {
        title: title,
        discription: desc,
        topic: Topic,
        keyWord: keyword,
  ...
  return (
    <>
        ...
        <Col span={24}>
          <Card style={{ minHeight: "80vh", height: "100%" }}>
            ...
            <Row gutter={24}>
              ...
              <Col span={6}>
                <Button onClick={showVerifyModal}loading={loading} type="primary">
                  Preview
                </Button>
              </Col>
            </Row>
      ...
      <Modal
        ...
        confirmLoading={onEdit ? saveLoading : uploadLoading}
        okButtonProps={{
          loading: onEdit ? saveLoading : uploadLoading, // Button-specific loading
        }}
        style={{
          overflow: "auto",
          height: 500,
